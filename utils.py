...
@author: pleiades
@reference: infpy by John Reid 2006
'''
...
def lu_inv( L ):
    ...
    from numpy import zeros_like, matrix, eye
    n = L.shape[0]
...
def matrix_from_function( f, shape, dtype, symmetric = False ):
    """
    [input] f: covar function
            shape: e.g. ( len( x1 ), len( x2 ) )
...
def chol(A, ztol= 1.0e-5):
    """
    Computes the upper triangular Cholesky factorization of  
    a positive definite matrix A.
    
    #Read more: http://adorio-research.org/wordpress/?p=4560#ixzz0rtjjxpAz

    """
    # Forward step for symmetric triangular t.
    #print "[chol] Given input: \n", A 
    nrows = len(A)
    #print "[chol] nrows: ", nrows
    #t = matzero(nrows, nrows)
    t = numpy.matrix(numpy.zeros((nrows,nrows), numpy.float64))
    for i in range(nrows):
        #S = sum([(t[k][i]) ** 2 for k in range(i)])
        S = sum([(t[k,i]) ** 2 for k in range(i)])
        #print "[chol] S = %s" % str(S)
        #d = A[i][i] - S
        
        d = A[i,i] - S
        
        #print "[chol] d = ", d
        #print "[chol] A[i][i]: %s  | A[i,i]: %s" % (A[i][i], A[i,i])
...
def check_gradients( f, fprime, x, eps = 1e-4 ):
    ...
    print "[check_gradients] gradJ =  %s" % calculated
    print "              VS  approx = %s" % approximation
    
    diff = calculated - approximation
    print "[check_gradients] diff = %s" % diff
